...
if [ $# -lt 1 ]
then
	echo "Using default directory ${OUTDIR} for outpgit submodule update --init --recursiveut"
else
	OUTDIR=$1
...
mkdir ${OUTDIR}/rootfs
cd ${OUTDIR}/rootfs
mkdir -p bin sbin dev etc home lib lib64 proc sys tmp usr var usr/bin usr/lib usr/sbin var/log
